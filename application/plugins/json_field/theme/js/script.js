var JSON_FIELD = function(clazz){
	this.currentClazz = clazz;
	this.baseurl = $('base').attr("href");
	this.admincp = 'Techsystem';
	var self1 = this;
	this.uniqueID = function () {
      return 'json_field_' + Math.random().toString(36).substr(2, 9);
    };
	this.initAddItem = function(){
		var self = this;
		$('.json_field .add-'+self.currentClazz).click(function(event) {
			event.preventDefault();
			var html = $(this).closest('.json_field').find('.hidden-item').html();
			var fdivs = $(html).find('div[class^=image]');
			for(var i = 0;i<fdivs.length;i++){
                var div = $(fdivs[i]);
                var oldId = div.attr('class');
			    html = html.replace(new RegExp(oldId, 'g'), self.uniqueID());
            }
			
			$('.json_field .list-items-'+self.currentClazz).append(html);
			self.initEditor();
		});
	}
	this.initCloseItem = function(){
		var self = this;
		$(document).on('click', '.json_field .list-items-'+self.currentClazz+' span.close', function(event) {
			event.preventDefault();
	...
	this.initChange = function(){
		var self = this;
		$(document).on('input', '.json_field .list-items-'+self.currentClazz+' .item .text,.json_field .list-items-'+self.currentClazz+' .item .textarea,.json_field .list-items-'+self.currentClazz+' .item .number,.json_field .list-items-'+self.currentClazz+' .item .checkbox', function(event) {
			event.preventDefault();
			self.getDataValue();
		});
	}
	this.getDataValue = function (){
		...
		$('#'+this.currentClazz).val(JSON.stringify({...objs}));
	}
	    	...
	    	buttons: {
		        success: {
		        	...
		        	callback: function () {
						...
						window['json_field_'+tmpClazz].getDataValue();
		          	}
		        }
	      	}
	    });
	...
	this.deleteImage = function (id){
		$('img.'+id).attr('src','theme/admin/images/noimage.png');
		$('textarea.'+id).val('');
		this.getDataValue();
	}
	...
	this.initEditor = function(){
		...
		for (var i = 0; i < items.length; i++) {
			...
			var options = this.mergeArray({
			    ...
			    toolbar2:"formatselect fontselect fontsizeselect lineheightselect | link unlink table | forecolor backcolor | tech5sfilemanager | image",
			    toolbar3:"",
			    sticky_offset: 45,
			    image_advtab: true ,
			    style_formats: [
			    ...
			    external_plugins: { "filemanager" : this.baseurl+"public/tinymce/plugin.min.js"},
...
var JSON_FIELD_PROVIDER = (function(){
